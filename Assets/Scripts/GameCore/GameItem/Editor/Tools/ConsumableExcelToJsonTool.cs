...
public static class ConsumableExcelToJsonTool
{
    private const string PreferredSheetName = "Consumable";
    private const string OutputRelativePath = "Assets/Resources/GameData/ItemData/ConsumableData.json";
    private const string DefaultExcelFolderRelativePath = "Assets/Resources/Excel";

    [MenuItem("Tools/Game Item/Import Consumable Excel To Json")]
    public static void ImportConsumableExcelToJson()
    {
        string excelPath = EditorUtility.OpenFilePanel("Select Consumable Excel File", GetDefaultExcelFolder(), "xlsx");
        ...
        try
        {
            ...
            ConsumableJsonCollection collection = BuildCollection(rows);
            ...
            Debug.Log($"[ConsumableExcelToJsonTool] Exported {collection.items.Count} consumable records to {OutputRelativePath}");
            EditorUtility.DisplayDialog("Export Completed", $"消耗品表已导出到:\n{OutputRelativePath}", "OK");
        }
        catch (Exception ex)
        {
            Debug.LogError($"[ConsumableExcelToJsonTool] Failed to import consumable excel.\n{ex}");
    ...
    private static ConsumableJsonCollection BuildCollection(List<Dictionary<string, string>> rows)
    {
        ConsumableJsonCollection collection = new ConsumableJsonCollection();
        ...
        for (int i = 0; i < rows.Count; i++)
        {
            ...
            ConsumableJsonRecord record = new ConsumableJsonRecord
            {
                itemId = itemId,
                ...
                maxStack = ReadInt(row, "maxStack"),
                recoverHp = ReadInt(row, "recoverHp"),
                recoverMp = ReadInt(row, "recoverMp"),
                buffIDs = ReadIntArray(row, "buffIds", "buffIDs", "buffId", "buffID")
            };
    ...
    private static string BuildFunctionDescription(ConsumableJsonRecord record)
    {
        List<string> parts = new List<string>();

        AppendFlatStat(parts, "恢复生命值", record.recoverHp);
        AppendFlatStat(parts, "恢复魔法值", record.recoverMp);
        if (record.buffIDs != null && record.buffIDs.Length > 0)
        {
            string ids = string.Join(",", record.buffIDs);
            parts.Add($"附加 Buff #{ids}");
        }
    ...
    private static readonly char[] IntArraySeparators = { ',', '，', ';', '；', '|', ' ', '/' };

    /// <summary>
    /// 读一列形如 "1001,1005" / "1001 1005" 的字符串，拆成 int[]。0 或非数字会被过滤。
    /// </summary>
    private static int[] ReadIntArray(Dictionary<string, string> row, params string[] keys)
    {
        ...
        string[] parts = raw.Split(IntArraySeparators, StringSplitOptions.RemoveEmptyEntries);
        List<int> result = new List<int>(parts.Length);
        for (int i = 0; i < parts.Length; i++)
        {
            string trimmed = parts[i].Trim();
            if (int.TryParse(trimmed, NumberStyles.Integer, CultureInfo.InvariantCulture, out int value) && value > 0)
            {
                result.Add(value);
            }
        }
