...
public static class MaterialExcelToJsonTool
{
    private const string PreferredSheetName = "Material";
    private const string OutputRelativePath = "Assets/Resources/GameData/ItemData/MaterialData.json";
    private const string DefaultExcelFolderRelativePath = "Assets/Resources/Excel";

    [MenuItem("Tools/Game Item/Import Material Excel To Json")]
    public static void ImportMaterialExcelToJson()
    {
        string excelPath = EditorUtility.OpenFilePanel("Select Material Excel File", GetDefaultExcelFolder(), "xlsx");
        ...
        try
        {
            ...
            MaterialJsonCollection collection = BuildCollection(rows);
            ...
            Debug.Log($"[MaterialExcelToJsonTool] Exported {collection.items.Count} material records to {OutputRelativePath}");
            EditorUtility.DisplayDialog("Export Completed", $"材料表已导出到:\n{OutputRelativePath}", "OK");
        }
        catch (Exception ex)
        {
            Debug.LogError($"[MaterialExcelToJsonTool] Failed to import material excel.\n{ex}");
    ...
    private static MaterialJsonCollection BuildCollection(List<Dictionary<string, string>> rows)
    {
        MaterialJsonCollection collection = new MaterialJsonCollection();
        ...
        for (int i = 0; i < rows.Count; i++)
        {
            ...
            MaterialJsonRecord record = new MaterialJsonRecord
            {
                itemId = itemId,
