...
public static class DLIOUtility
{
    ...
    private static void SaveNodeToGraph(DLNode node, DLGraphSaveDataSO graphData)
    {
        ...
        DLNodeSaveData nodeData = new DLNodeSaveData()
        {
            ...
            TaskDataSO = node.TaskDataSO,
            TaskID = node.TaskID,
            TaskStepID = node.TaskStepID,
            CustomEventName = node.CustomEventName
        };
    ...
    private static void SaveNodeToScriptableObject(DLNode node, DLContainerSO container)
    {
        ...
        dialogue.Initialize(
            ...
            node.DialogueType,
            ShouldExportAsStartingDialogue(node)
        );
        dialogue.SetEventData(
            node.EventType,
            node.TaskDataSO,
            node.TaskID,
            node.TaskStepID,
            node.CustomEventName
        );
    ...
    private static void UpdateDlChoicesConnections()
    {
        foreach(DLNode node in nodes)
        {
            ...
            for(int choiceIndex = 0; choiceIndex < node.Choices.Count; ++choiceIndex)
            {
                ...
                {
                    Debug.LogWarning($"[DLIOUtility] Dialogue node '{node.DialogueName}' choice '{nodeChoice.Text}' has no connected target node. Exported runtime choice will stop the dialogue.");
                    continue;
                }

                if (!createdDialogues.ContainsKey(nodeChoice.NodeID))
                {
                    Debug.LogWarning($"[DLIOUtility] Dialogue node '{node.DialogueName}' choice '{nodeChoice.Text}' points to missing node id '{nodeChoice.NodeID}'. Exported runtime choice will stop the dialogue.");
                    continue;
                }
    ...
    private static bool ShouldExportAsStartingDialogue(DLNode node)
    {
        if (node == null || node is DLStartNode)
        {
            return false;
        }

        bool hasExplicitStartNodeInScope = nodes.Any(otherNode =>
            otherNode is DLStartNode &&
            otherNode.Group == node.Group);

        return hasExplicitStartNodeInScope
            ? node.HasStartNodeInput()
            : node.IsStartingNode();
    }
    ...
    private static void LoadNodes(List<DLNodeSaveData> nodes)
    {
        foreach(DLNodeSaveData nodeData in nodes)
        {
            ...
            node.EventType = nodeData.EventType;
            node.TaskDataSO = nodeData.TaskDataSO;
            node.TaskID = nodeData.TaskID;
            node.TaskStepID = nodeData.TaskStepID;
    ...
    private static void LoadNodesConnections()
    {
        foreach(KeyValuePair<string, DLNode> loadedNode in loadedNodes)
        {
            foreach (Port choicePort in loadedNode.Value.outputContainer.Children())
            {
                ...
                if (!loadedNodes.TryGetValue(choiceData.NodeID, out DLNode nextNode))
                {
                    continue;
                }

                Port nextNodeInputPort = (Port) nextNode.inputContainer.Children().OfType<Port>().SingleOrDefault();
