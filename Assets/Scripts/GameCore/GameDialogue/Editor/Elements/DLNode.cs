...
public class DLNode : Node
{
    ...
    public DialogueEventType EventType { get; set; }
    public TaskDataSO TaskDataSO { get; set; }
    ...
    public bool HasStartNodeInput()
    {
        foreach (Port inputPort in inputContainer.Children().OfType<Port>())
        {
            foreach (Edge edge in inputPort.connections)
            {
                if (edge?.output?.node is DLStartNode)
                {
                    return true;
                }
            }
